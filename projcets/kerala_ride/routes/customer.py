...
# ==========================================
# 1. CUSTOMER BOOKING PAGE ROUTE (MULTIPLE STOPS ENABLED)
# ==========================================
...
def book():
    if request.method == 'POST':
        try:
            ...
            stopover = request.form.get('stopover', '').strip()
            destination = request.form.get('destination', '').strip()
            
            # Resolve combined field tags matching layout selectors
            ...
            # Extract numeric coordinate fields seamlessly
            p_lat_raw = request.form.get('pickup_lat', '').strip()
            p_lng_raw = request.form.get('pickup_lng', '').strip()
            s_lat_raw = request.form.get('stopover_lat', '').strip()
            s_lng_raw = request.form.get('stopover_lng', '').strip()
            ...
            stopover_lat = float(s_lat_raw) if s_lat_raw else None
            ...
            new_booking = Booking(
                ...
                stopover_location=stopover if stopover else None,
                ...
                stopover_lat=stopover_lat,
                stopover_lng=stopover_lng,
                dest_lat=dest_lat,
                dest_lng=dest_lng,
            ...
            # --- 📲 STEP 2: CELERY CLOUD TASK DISPATCH ---
            try:
...
def estimate_fare():
    ...
    try:
        ...
        s_lat = data.get('stopover_lat')
        s_lng = data.get('stopover_lng')
        ...
        s_lat = float(s_lat) if s_lat else None
        s_lng = float(s_lng) if s_lng else None
    ...
    try:
        if s_lat and s_lng:
            url = f"http://router.project-osrm.org/route/v1/driving/{p_lng},{p_lat};{s_lng},{s_lat};{d_lng},{d_lat}"
        else:
            url = f"http://router.project-osrm.org/route/v1/driving/{p_lng},{p_lat};{d_lng},{d_lat}"
        ...
        if response.status_code == 200 and res_data.get('code') == 'Ok':
            ...
            raise Exception("OSRM API chain split error code.")

    except Exception as e:
        print(f"⚠️ OSRM Engine Unavailable ({str(e)}). Engaging sequential Haversine fallback matrix nodes.")
        if s_lat and s_lng:
            dist1 = calculate_straight_line_distance(p_lat, p_lng, s_lat, s_lng)
            dist2 = calculate_straight_line_distance(s_lat, s_lng, d_lat, d_lng)
            distance_km = round(dist1 + dist2, 1)
            route_geometry = {
                "type": "LineString",
                "coordinates": [[p_lng, p_lat], [s_lng, s_lat], [d_lng, d_lat]]
            }
        else:
    ...
    passenger_categories = ["Auto Rickshaw", "Taxi", "Standard Cars", "8-Seater SUV", "MPV", "Tempo Traveller"]
    goods_categories = ["Goods Auto", "Mini Pickup", "Pickup", "Lorry"]
    
    target_categories = goods_categories if category in goods_categories else passenger_categories
    estimates = {}
    selected_estimate = None

    for cat in target_categories:
        if cat == "Auto Rickshaw":
            base_f, min_km, rate_km = 40.0, 1.5, 12.0
        elif cat == "Taxi":
            base_f, min_km, rate_km = 60.0, 3.0, 15.0
        elif cat == "8-Seater SUV":
            base_f, min_km, rate_km = 100.0, 5.0, 22.0
        elif cat == "Goods Auto":
            base_f, min_km, rate_km = 80.0, 2.0, 18.0
        elif cat in ["Mini Pickup", "Mini Truck"]:
            base_f, min_km, rate_km = 150.0, 5.0, 25.0
        else:
            base_f, min_km, rate_km = 60.0, 3.0, 15.0

        try:
            fare_config = FareConfig.query.filter_by(vehicle_category=cat).first()
            if fare_config:
                base_f = float(fare_config.base_fare)
                min_km = float(fare_config.base_distance_km)
                rate_km = float(fare_config.rate_per_km)
        except Exception as db_err:
            print(f"FareConfig error for {cat}: {db_err}")

        cat_is_outstation = False
        cat_return_charge = 0.0
        cat_forward_fare = 0.0

        if distance_km <= min_km:
            cat_forward_fare = base_f
            cat_return_charge = 0.0
        else:
            billable_extra_km = distance_km - min_km
            cat_forward_fare = base_f + (billable_extra_km * rate_km)
            
            if distance_km > 500.0:
                cat_is_outstation = True
                cat_return_charge = distance_km * rate_km
            else:
        ...
        if promo_code:
            try:
                ...
                if promo and not promo.is_expired:
                    cat_discount = round(cat_final_fare * (promo.discount_percentage / 100.0), 2)
                    cat_final_fare -= cat_discount
            except Exception as promo_err:
                print(f"Promo error for {cat}: {promo_err}")

        cat_final_fare = max(0.0, round(cat_final_fare, 2))

        est_details = {
            "base_fare": base_f,
            "base_distance": min_km,
            "rate_per_km": rate_km,
            "return_charge": cat_return_charge,
            "waiting_charge": waiting_charge_total,
            "discount": cat_discount,
            "final_fare": cat_final_fare,
            "is_outstation": cat_is_outstation
        }
        
        estimates[cat] = est_details
        if cat == category:
            selected_estimate = est_details

    if not selected_estimate:
        selected_estimate = {
            "base_fare": 60.0, "base_distance": 3.0, "rate_per_km": 15.0,
            "return_charge": 0.0, "waiting_charge": 0.0, "discount": 0.0, "final_fare": 0.0, "is_outstation": False
        }

    return jsonify({
        "success": True,
        "distance_km": distance_km,
        "base_fare": selected_estimate["base_fare"],
        "base_distance": selected_estimate["base_distance"],
        "rate_per_km": selected_estimate["rate_per_km"],
        "return_charge": selected_estimate["return_charge"],
        "waiting_charge": selected_estimate["waiting_charge"],
        "discount": selected_estimate["discount"],
        "final_fare": selected_estimate["final_fare"],
        "promo_applied": True if selected_estimate["discount"] > 0 else False,
        "is_outstation": selected_estimate["is_outstation"],
        "estimates": estimates,
